...
parserFactory.registerRule(
	(url) => MediumParser.isMedium(url),
	() => new MediumParser()
);

class MediumImageCollector extends ImageCollector {
	constructor() {
		super();
	}
	...
	extractWrappingUrl(element) {
		...
		const source = element.parentElement.querySelector("source");

		if (url === "" && source !== null) {
			const srcset = source.getAttribute("srcset");
			if (srcset !== null) {
				url = this.findHighestResInSrcset(srcset)
			}
		}
	...
	findImagesUsedInDocument(content) {
		for(let imageElement of content.querySelectorAll("img")) {
			...
			this.addImageInfo(wrappingUrl, wrappingUrl, null, false);
		};
	}
...
class MediumParser extends Parser{
	constructor() {
		super(new MediumImageCollector());
	...
	static isMedium(url) {
		return (util.extractHostName(url).indexOf(".medium.com") !== -1)
	}
	...
	*/
	...
	async getChapterUrls(dom) {
		return [...dom.querySelectorAll("article div[role=link]")]
		.map(article => {
			return {
				sourceUrl: article.dataset.href,
	...
	extractTitleImpl(dom) {
		return dom.querySelector("head title").textContent;
	}
	...
	*/
	...
	removeUnwantedElementsFromContentElement(element) {
		// remove clap and comment count
		util.removeChildElementsMatchingCss(element, "h1 + div > div > div > div:nth-child(2)");
